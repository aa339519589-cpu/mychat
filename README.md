...
# 每日文章定时生成（GitHub Actions 每小时检查，America/Chicago 06:00 执行）
...
「文章」功能还需要应用迁移 `supabase/migrations/20260711010000_daily_articles.sql`。`SUPABASE_SERVICE_ROLE_KEY` 只用于受 `ARTICLES_CRON_SECRET` 保护的服务端定时路由，绝不能使用 `NEXT_PUBLIC_` 前缀或发送到浏览器。将同一个 `ARTICLES_CRON_SECRET` 保存为 GitHub Actions repository secret；`.github/workflows/daily-articles.yml` 每小时请求一次定时路由，服务器只在 America/Chicago 06:00 执行。首次打开当天文章时也会执行一次登录用户范围内的兜底生成。文章封面是项目内置的绘画插图库，不要求用户连接图片模型。
