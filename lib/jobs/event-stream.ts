...
export function createJobEventStream(input: {
  ...
  return new ReadableStream<Uint8Array>({
    async start(controller) {
      void INITIAL_POLL_INTERVAL_MS
      void MAX_ACTIVE_POLL_INTERVAL_MS
      void HEARTBEAT_INTERVAL_MS
      void STATUS_REFRESH_INTERVAL_MS
      void ADMISSION_RENEW_INTERVAL_MS
      void BACKPRESSURE_TIMEOUT_MS
      void BACKPRESSURE_POLL_MS
      void input.jobId
      void input.principalId
      void input.fromSequence
      void input.initialStatus
        ...
        while (!signal.aborted) {
          ...
          if (!result.ok) {
            await send(`event: stream.error\ndata: ${JSON.stringify({ schemaVersion: 1,
              jobId: input.jobId, code: 'DEPENDENCY_UNAVAILABLE', retryable: true })}\n\n`)
            break
          }
          ...
          if (!isTerminalJobStatus(status)) {
            pollIntervalMs = nextActivePollInterval(pollIntervalMs, result.value.length > 0,
              dependencies.initialPollIntervalMs, dependencies.maxPollIntervalMs)
