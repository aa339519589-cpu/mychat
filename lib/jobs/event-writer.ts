...
function eventDraft(event: ChatEvent): JobEventDraft | null {
  ...
  if ('imageSummary' in event) return { kind: 'context.image_summary', payload: { imageSummary: jsonObject(event.imageSummary) } }
...
function deltaValue(event: JobEventDraft): { field: 'text' | 'thinking'; value: string } | null {
  if (event.kind === 'text.delta' && typeof event.payload.text === 'string') return { field: 'text', value: event.payload.text }
  if (event.kind === 'thinking.delta' && typeof event.payload.thinking === 'string') return { field: 'thinking', value: event.payload.thinking }
  return null
}

function materializedText(progress: JsonObject | undefined, field: 'content' | 'thinking', partsField: 'contentParts' | 'thinkingParts'): string {
  ...
  for (const part of parts) {
    if (!part || typeof part !== 'object' || Array.isArray(part) || part.type !== 'text' || typeof part.text !== 'string') return ''
    text.push(part.text)
  }
  return text.join('')
}
...
export class JobEventWriter {
  ...
  emit = (event: ChatEvent): void => {
    ...
    if (current && previous && previousDelta?.field === current.field && previousDelta.value.length + current.value.length <= MAX_COALESCED_DELTA_CHARS) {
      previous.payload[current.field] = `${previousDelta.value}${current.value}`
    } else this.queue.push(draft)
    if (isText && !this.firstTextFlushed) {
      this.firstTextFlushed = true
      this.scheduleFlush(0)
    } else if (this.queue.length >= FLUSH_BATCH_SIZE) this.scheduleFlush(0)
  ...
  text(): string { return this.fullText }
  thinking(): string { return this.fullThinking }
  ...
  async checkpoint(input: { phase: string; data: JsonObject; resumable: boolean; extraProgress?: JsonObject }): Promise<void> {
  ...
  private scheduleFlush(milliseconds: number): void {
    if (this.timer) clearTimeout(this.timer)
    this.timer = setTimeout(() => { this.timer = null; void this.flush() }, milliseconds)
  }
