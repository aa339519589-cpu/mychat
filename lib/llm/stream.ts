...
export function userFacingUpstreamErrorMessage(value: string): string {
  const message = value.replace(/\s+/g, ' ').trim()
  if (/ERR_NGROK_3200|endpoint\s+\S*ngrok(?:-free)?\.\S*\s+is\s+offline/i.test(message)) {
    return '上游 ngrok 反代隧道已离线（ERR_NGROK_3200）。请重启隧道，或在部署环境中把对应的 DEEP_TIER_*_BASE_URL 改为长期可用的 HTTPS 地址。'
  }
  return value
}
...
export function send(controller: ReadableStreamDefaultController, data: object) {
  const payload = 'error' in data && typeof data.error === 'string'
    ? { ...data, error: userFacingUpstreamErrorMessage(data.error) }
    : data
...
export function upstreamError(status: number, raw: string, source = '模型服务', exactSecrets: readonly string[] = []) {
  ...
  return userFacingUpstreamErrorMessage(`${source}请求失败（${status}）：${message.slice(0, 180) || '未返回原因'}`)
}
...
export function networkError(error: unknown, source = '模型服务', exactSecrets: readonly string[] = []) {
  ...
  return userFacingUpstreamErrorMessage(`${source}请求失败：${message}`)
}
