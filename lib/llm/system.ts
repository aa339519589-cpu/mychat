...
【可视化渲染】如果工具库没有，根据内容性质选择以下三种方式之一：

▌方式一：Vega-Lite 图表（用 <vega> 标签，优先用这个）
适用于：折线图、柱状图、散点图、饼图、面积图等常见统计图表。
...
▌方式二：内联 SVG（用 <inline-artifact> 标签）
...
方式二：内联 SVG（用 <inline-artifact> 标签）
适用于：纯手画坐标的简单图形（五线谱、几何图、函数曲线等），Vega-Lite 不支持的特殊图表。
自己算好所有坐标点写进去，库不负责排版。
