...
async function understandImages(images: string[], prompt: string, maxTokens: number, parentSignal?: AbortSignal): Promise<string> {
  if (!MIMO_API_KEY) {
    log.warn('mimo', 'MIMO_API_KEY not configured')
    return ''
  }
  try {
    const signals = [parentSignal, AbortSignal.timeout(120_000)].filter(Boolean) as AbortSignal[]
...
// 一批页面图片（data URL）→ 文字
async function ocrChunk(images: string[], signal?: AbortSignal): Promise<string> {
  return understandImages(images, OCR_PROMPT, 16_384, signal)
}

export async function summarizeImages(images: string[], signal?: AbortSignal): Promise<string> {
  return understandImages(images.slice(0, 8), SUMMARY_PROMPT, 1_024, signal)
}

// 渲染好的页面图片（前端 pdfjs 生成的 data URL 数组）→ 完整文字。分块并行，控制延迟。
export async function ocrPageImages(images: string[], signal?: AbortSignal): Promise<string> {
  ...
  const results = await Promise.all(chunks.map(chunk => ocrChunk(chunk, signal)))
