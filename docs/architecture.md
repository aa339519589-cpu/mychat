...
`npm run quality` runs the architecture check, the closed migration-manifest contract, strict TypeScript, zero-warning backend and project-wide ESLint gates, and unit/integration tests with minimum exercised-module coverage gates of 80% lines, 80% branches, and 80% functions. Warnings fail the build in every production, browser, script, and test path. The architecture checker also rejects library modules that are unreachable from runtime entry points, prevents type-only imports from inflating runtime dependency budgets, and exempts oversized generated source only when it is explicitly declared and carries a generated-code marker. `npm run verify` adds a production dependency audit, deterministic database-type drift verification, an actual PostgreSQL 16 baseline/expand/replay/contract migration verification (including concurrent rate-limit contention and the immutable schema attestation), a production build, and Playwright smoke tests against `next start` on desktop Chromium and a Pixel 7 viewport. Pull requests and pushes to `main` run this same complete gate in CI.
...
`scripts/start-production.ts` 支持三种 `MYCHAT_RUNTIME_ROLE`：
...
只有具有仓库内持久消费者的副作用才进入可交付 outbox。当前 deliverable topic 是 `assets.cleanup` 与 `payloads.cleanup`，它们和业务状态在同一数据库事务写入；Dispatcher 使用独立 lock version、续租和 CAS acknowledgement 执行私有 Storage 清理。`jobs.ready`、`jobs.terminal`、`jobs.cancel_requested` 与 `jobs.poison` 的权威事实已经存在于 Job、event 和 audit 表，v2 trigger 会抑制这些没有外部消费者的伪发布行；对应指标直接从权威记录计算。仓库尚未接入 Kafka、webhook 或第三方 event bus，未来新增 topic 前必须先提供真实消费者、幂等键和失败恢复契约。
...
每个 Worker 进程每 5 秒通过一次 `heartbeat_job_worker_v2` 写入 revision、逐队列精确容量、启动时间和 draining 状态，不再为五个队列分别发送 heartbeat。`read_job_worker_readiness_v3` 只汇总同一 revision、20 秒内、非 draining 的进程，并按每个队列自己的容量判断覆盖。`/api/ready` 要求全部五个固定队列满足该契约；Web 进程不会用自己的内存计数冒充跨进程 Worker 健康。`/api/metrics` 同样从数据库读取 fleet 与 Job 权威快照，并在数据库或解析失败时返回 `503`。

`MYCHAT_MAINTENANCE_MODE=drain` 是维护开关，旧的 `GENERATION_MAINTENANCE_MODE=true` 仍作为兼容别名。drain 会在读取大请求体和执行昂贵命令前拒绝新的 Agent、发布与 `awaiting_input` 恢复请求；普通聊天、媒体和标题任务继续由受限 Worker 池处理。Agent 与 outbox 不会被 claim，状态读取和取消仍可使用。维护实例可保持读流量 ready，但返回体显式标记 `checks.worker.draining=true`；生产发布校验脚本会拒绝把 draining revision 当作完成部署。
...
- 浏览器只提交带预期尾消息 fence 的 turn authority，不直接插入、更新或删除消息。服务端 `enqueue_chat_turn_v1` 在一个事务内创建可选会话、用户消息、assistant placeholder、Job 与 generation 投影；`enqueue_chat_regeneration_v1` 以 CAS 原子替换分支并登记媒体清理。任一检查失败时旧分支保持不变。Worker 在调用 provider 前必须持有 Job fence。
