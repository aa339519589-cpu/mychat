...
test('worker entrypoint advertises one process heartbeat with exact per-queue capacities', () => {
  ...
  assert.match(workerEntrypoint, /createWorker\(spec, 'chat-hot', 1, true\)/)
  assert.match(workerEntrypoint, /remainingConcurrency = spec\.concurrency - 1/)
  assert.match(workerEntrypoint, /createWorker\(spec, 'chat-bulk', remainingConcurrency\)/)
  assert.match(workerEntrypoint, /idleBackoffMinimumMs: 20/)
  assert.match(workerEntrypoint, /idleBackoffMaximumMs: 150/)
