...
test('worker entrypoint advertises each real consumer with only its own capacity', () => {
  assert.match(workerEntrypoint, /const heartbeatSpecs = \[/)
  assert.match(workerEntrypoint, /\{ name: 'outbox', queue: 'outbox', capacity: 1 \}/)
  assert.match(workerEntrypoint, /workerId: `\$\{baseWorkerId\}:\$\{spec\.name\}`/)
  assert.match(workerEntrypoint, /queues: \[spec\.queue\]/)
  assert.match(workerEntrypoint, /capacity: spec\.capacity/)
  assert.match(workerEntrypoint, /Promise\.all\(\[[\s\S]*?\.\.\.heartbeats\.map\(heartbeat => heartbeat\.run/)
  assert.match(workerEntrypoint, /billingReconciliation\.run\(shutdown\.signal\)/)
  assert.doesNotMatch(workerEntrypoint, /capacity:\s*workerSpecs\.reduce/)
  assert.doesNotMatch(workerEntrypoint, /queues:\s*\[\.\.\.workerSpecs\.flatMap/)
})
