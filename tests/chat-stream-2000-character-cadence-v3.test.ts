...
test("active Chat polling stays below 80 ms", () => {
  let interval = 35
  for (let index = 0; index < 100; index += 1) {
    interval = nextActivePollInterval(interval, false)
    assert.ok(interval <= 80)
  }
  assert.equal(nextActivePollInterval(80, true), 35)
})

test("Chat delivers a 2200-character response continuously", async () => {
  ...
  const stream = createJobEventStream({
    ...
    readJob: async () => ({
      ok: true as const,
      value: { status: cursor >= events.length ? "completed" : "running" } as never,
    }),
