...
test('release promotion ties one verified SHA to the image digest and exact Render deploy', () => {
  ...
  assert.match(activation, /workflow_call:/)
  assert.match(activation, /release_in_progress:[\s\S]*?type:\s*boolean/)
  assert.match(activation, /group:\s*\$\{\{ inputs\.release_in_progress && format\('production-activation-\{0\}', github\.run_id\) \|\| 'production-release-main' \}\}/)
  assert.match(activation, /cancel-in-progress:\s*false\s*\n\s*queue:\s*max/)
  assert.match(activation, /DRAIN_RELEASE_RUN_ID:\s*\$\{\{ inputs\.drain_release_run_id \}\}/)
  assert.match(activation, /REQUESTED_REVISION:\s*\$\{\{ inputs\.revision \}\}/)
  assert.match(activation, /\$releaseInProgress and \.status == "in_progress"/)
  assert.match(activation, /\$releaseInProgress \| not/)
  ...
  assert.match(release, /^  activate:[\s\S]*?uses:\s*\.\/\.github\/workflows\/activate-production\.yml/m)
  assert.match(release, /release_in_progress:\s*true/)
  assert.match(release, /drain_release_run_id:\s*\$\{\{ format\('\{0\}', github\.run_id\) \}\}/)
  assert.match(release, /group:\s*production-release-main\s*\n\s*cancel-in-progress:\s*false\s*\n\s*queue:\s*max/)
