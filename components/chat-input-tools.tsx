...
import type { CustomReasoningMode } from "@/lib/model-reasoning"
...
export function ComposerTools({
  ...
  reasoningEffort, reasoningOptions, reasoningMode, onReasoningChange,
}: {
  open: boolean
  ...
  return (
    <div ref={containerRef} className="relative order-2 flex shrink-0 items-center gap-0.5">
      ...
      <ReasoningTool value={reasoningEffort} options={reasoningOptions} mode={reasoningMode} onChange={onReasoningChange} reducedMotion={reducedMotion} />
    </div>
  )
}

function ReasoningTool({ value, options, mode, onChange, reducedMotion }: { value: string | null; options: string[]; mode?: CustomReasoningMode | null; onChange: (value: string) => void; reducedMotion: boolean | null }) {
  ...
  return (
    <div className="relative">
      <button type="button" disabled={disabled} onClick={() => setMenuOpen(!menuOpen)} aria-label={mode === "budget" ? "Thinking token budget" : "Thinking depth"} aria-expanded={disabled ? undefined : menuOpen} className={cn("fluid-press fluid-icon-press flex size-11 items-center justify-center rounded-full transition-colors", disabled ? "cursor-not-allowed text-muted-foreground/25" : active ? "bg-secondary text-foreground" : "text-[#B0B0B0] hover:bg-secondary/70 hover:text-foreground dark:text-white/40 dark:hover:bg-white/10 dark:hover:text-white")}><Brain className="size-[1.15rem]" /></button>
      <AnimatePresence initial={false}>{menuOpen && !disabled && <motion.div initial={reducedMotion ? { opacity: 0 } : { opacity: 0, y: 5, scale: 0.96 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={reducedMotion ? { opacity: 0 } : { opacity: 0, scale: 0.97, y: 4 }} transition={transitionFor(reducedMotion, POPOVER_SPRING)} className="fluid-material-strong absolute bottom-full left-1/2 z-30 mb-2 min-w-[8.5rem] -translate-x-1/2 overflow-hidden rounded-2xl border border-border/70 p-1.5 shadow-lg">{orderedOptions.map(option => <button key={option} type="button" onClick={() => { onChange(option); setMenuOpen(false) }} className="fluid-press flex min-h-10 w-full items-center justify-between rounded-xl px-3 text-left text-[12px] text-foreground hover:bg-secondary/70"><span>{reasoningLabel(option, mode)}</span>{value === option && <Check className="size-3.5" />}</button>)}</motion.div>}</AnimatePresence>
    </div>
  )
}

function reasoningLabel(value: string, mode?: CustomReasoningMode | null): string {
  if (mode === "budget") return BUDGET_LABELS[value] ?? value
