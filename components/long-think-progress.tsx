...
export function LongThinkProgress({ job }: { job: JobSnapshot | null }) {
  ...
  return <div className="mt-5 rounded-2xl border border-border/70 bg-background/55 p-4 sm:p-5">
    ...
    {reasoning ? <div className="mt-3 max-h-[26rem] overflow-auto whitespace-pre-wrap break-words rounded-xl border border-border/60 bg-background p-3 text-sm leading-6"><div className="mb-2 text-[11px] text-muted-foreground">模型返回的 reasoning</div>{reasoning}</div> : <p className="mt-3 text-sm leading-6 text-muted-foreground">当前请求正在执行；接口本轮尚未返回可展示的 reasoning 字段。</p>}
