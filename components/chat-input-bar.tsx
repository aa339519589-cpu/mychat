...
export function ComposerBar({ mobile, value, onValueChange, textareaRef, onResize, onSubmit, disabled, isLoading, sendPending, activeTier, activeModelLabel, activeOutputKind, canSend, onStop, onOpenModel }: {
  ...
  return (
    <>
      ...
      <button type="button" onClick={onOpenModel} aria-label="选择模型" className="fluid-press mb-0.5 flex h-11 min-w-0 max-w-[6rem] shrink-0 items-center gap-1 rounded-[0.7rem] px-2 text-xs text-muted-foreground hover:bg-background/40 hover:text-foreground dark:hover:bg-white/10 sm:max-w-[11rem] sm:px-2.5">{outputIcon(activeOutputKind)}<span className="min-w-0 truncate">{activeModelLabel}</span><ChevronDown className="size-3 shrink-0" /></button>
      {isLoading ? <button onClick={onStop} aria-label="停止生成" className="fluid-press fluid-icon-press mb-0.5 flex size-11 shrink-0 items-center justify-center rounded-full bg-foreground text-background hover:opacity-80"><Square className="size-3.5 fill-current" /></button> : <button onClick={onSubmit} disabled={!canSend} aria-label="发送" className={cn("fluid-press fluid-icon-press mb-0.5 flex size-11 shrink-0 items-center justify-center rounded-full", canSend ? "bg-primary text-primary-foreground hover:opacity-90" : "cursor-not-allowed text-muted-foreground/30")}><ArrowUp className="size-4" /></button>}
    </>
  )
}
