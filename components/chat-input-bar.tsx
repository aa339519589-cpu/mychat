...
export function ComposerBar({ mobile, value, onValueChange, textareaRef, onResize, onSubmit, disabled, isLoading, sendPending, activeTier, activeModelLabel, activeOutputKind, canSend, onStop, onOpenModel }: {
  ...
  return (
    <>
      ...
      {isLoading ? <button type="button" onPointerDown={event => { if (event.button !== 0) return; event.preventDefault(); onStop() }} onClick={event => { if (event.detail === 0) onStop() }} aria-label="停止生成" className="fluid-press fluid-icon-press flex size-11 shrink-0 touch-manipulation items-center justify-center rounded-full bg-foreground text-background hover:opacity-80"><Square className="size-3.5 fill-current" /></button> : <button type="button" onClick={onSubmit} disabled={!canSend} aria-label="发送" className={cn("fluid-press fluid-icon-press flex size-11 shrink-0 items-center justify-center rounded-full", canSend ? "bg-primary text-primary-foreground hover:opacity-90" : "cursor-not-allowed text-muted-foreground/30")}><ArrowUp className="size-4" /></button>}
    </>
  )
}
