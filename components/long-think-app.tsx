...
import {
  LONG_THINK_ACTIVE as ACTIVE,
  LONG_THINK_STORAGE_KEY as STORAGE_KEY,
  formatLongThinkElapsed as formatElapsed,
  longThinkInteger as integer,
  longThinkJsonRequest as jsonRequest,
  longThinkNullableInteger as nullableInteger,
  longThinkResultAnswer as resultAnswer,
  type Endpoint,
  type JobSnapshot,
  type ListedJob,
} from "@/components/long-think-support"
...
export function LongThinkApp() {
  ...
  const loadEndpoints = useCallback(async () => {
    ...
    setEndpoints((body.endpoints ?? []).filter(endpoint => endpoint.outputKind === "chat"))
  }, [])
  ...
  const createEndpointIfNeeded = async (): Promise<string> => {
    ...
    const body = await jsonRequest("/api/endpoints", {
      ...
      body: JSON.stringify({ baseUrl: baseUrl.trim(), apiKey: apiKey.trim(), model: model.trim(), displayName: model.trim(), outputKind: "chat", authType: "auto" }),
  ...
  const start = async () => {
    ...
    try {
      ...
      setJob(null)
      await Promise.all([loadJob(body.jobId), loadJobs()])
    } catch (cause) {
  ...
  const stop = async () => {
    ...
    try {
      ...
      await Promise.all([loadJob(activeJobId), loadJobs()])
    } catch (cause) {
  ...
  const newTask = () => {
    setActiveJobId(""); setJob(null); setProblem(""); setError("")
    ...
    <main className="min-h-dvh bg-background text-foreground">
      <div className="mx-auto flex min-h-dvh w-full max-w-5xl flex-col px-4 pb-16 pt-[max(1rem,env(safe-area-inset-top))] sm:px-6">
        <header className="flex items-center gap-3 py-3">
          ...
          <div className="min-w-0"><div className="flex items-center gap-2"><BrainCircuit className="size-4" /><h1 className="font-heading text-xl font-semibold tracking-[-0.02em]">Long Think</h1></div><p className="mt-0.5 text-xs text-muted-foreground">后台连续处理，闭环后再交付最终答案</p></div>
          <button onClick={() => void loadJobs()} className="fluid-press ml-auto flex size-10 items-center justify-center rounded-full text-muted-foreground hover:bg-muted/60 hover:text-foreground" aria-label="刷新任务"><RefreshCw className="size-4" /></button>
        </header>
        ...
        {!activeJobId ? (
          <section className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
            <div className="rounded-[28px] border border-border/70 bg-card/55 p-4 sm:p-6">
              ...
              {selectedEndpoint ? (
                <div className="mt-3 rounded-2xl border border-border/60 bg-background/60 p-3 text-sm"><div className="font-medium">{selectedEndpoint.model}</div><div className="mt-1 truncate text-xs text-muted-foreground">{selectedEndpoint.baseUrl}</div>{selectedEndpoint.needsReconnect && <div className="mt-2 text-xs text-destructive">该连接需要重新配置凭据</div>}</div>
              ) : (
          ...
          <section className="mt-5 rounded-[30px] border border-border/70 bg-card/50 p-5 sm:p-7">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1"><h2 className="text-base font-semibold">{active ? "深度处理中" : job?.status === "completed" ? "已闭环" : job?.status === "cancelled" ? "已停止" : "任务结束"}</h2><div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-xs text-muted-foreground"><span>轮次 <b className="font-medium text-foreground">{round}</b></span><span>API 调用 <b className="font-medium text-foreground">{apiCalls}</b></span><span>运行 <b className="font-medium text-foreground">{formatElapsed(elapsed)}</b></span><span>输入 Token <b className="font-medium text-foreground">{inputTokens === null ? "未知" : inputTokens.toLocaleString()}</b></span><span>输出 Token <b className="font-medium text-foreground">{outputTokens === null ? "未知" : outputTokens.toLocaleString()}</b></span></div></div>
        ...
        <section className="mt-8">
          ...
          <div className="mt-2 divide-y divide-border/60 overflow-hidden rounded-2xl border border-border/70">
            {jobs.length === 0 ? <div className="p-4 text-sm text-muted-foreground">还没有长期任务</div> : jobs.slice(0, 12).map(item => {
              ...
              return <button key={item.id} onClick={() => { setActiveJobId(item.id); window.localStorage.setItem(STORAGE_KEY, item.id) }} className="fluid-press flex w-full items-center gap-4 bg-card/30 px-4 py-3 text-left hover:bg-muted/35"><div className="min-w-0 flex-1"><div className="truncate text-sm">{item.id.slice(0, 8)}</div><div className="mt-0.5 text-xs text-muted-foreground">轮次 {integer(p.round)} · API {integer(p.apiCalls)}</div></div><span className="text-xs text-muted-foreground">{ACTIVE.has(item.status) ? "处理中" : item.status === "completed" ? "已完成" : item.status === "cancelled" ? "已停止" : "失败"}</span></button>
            })}
          </div>
        </section>
      </div>
