...
export function LiteraryChat() {
  ...
  const syncConversationRoute = useEffectEvent(() => {
    ...
    // Explicit URL wins. Otherwise stay on the current root (usually a blank draft).
    ...
    // No matching root — keep a blank draft in focus instead of jumping to history.
    const existingDraft = items.find(item => item.draft && item.messages.length === 0)
    if (existingDraft) {
      rootConversationIdRef.current = existingDraft.id
      draftIdRef.current = existingDraft.id
      route.replaceConversation(null)
      if (existingDraft.id !== activeId) {
    ...
    rootConversationIdRef.current = id
    setConversations(previous => [draft, ...previous.filter(item => item.id !== id)])
    route.replaceConversation(null)
    setActiveId(id)
    activationTokenRef.current += 1
    setHydratingConversationId(null)
  ...
  async function handleDelete(id: string) {
    ...
    // After deleting the active chat, open a blank draft rather than the next history item.
    const existingDraft = remaining.find(conversation => conversation.draft && conversation.messages.length === 0)
    if (existingDraft) {
      rootConversationIdRef.current = existingDraft.id
      draftIdRef.current = existingDraft.id
      route.replaceConversation(null)
    ...
    setConversations(previous => [createDraft(draftId), ...previous])
    route.replaceConversation(null)
    setActiveId(draftId)
    activationTokenRef.current += 1
    setHydratingConversationId(null)
  ...
  const controller: LiteraryChatViewController = {
    ...
    chat: {
      ...
      input: { onSend: generation.handleSend, activeTier: model.activeTier, onTierChange: model.handleTierChange, models: model.catalog, endpoints: model.modelEndpoints, activeModelId: model.activeModelId, activeModel: model.activeModel, activeEndpointId: model.activeEndpointId, activeEndpoint: model.activeEndpoint, onModelChange: model.handleModelSelect, onEndpointChange: model.handleEndpointSelect, reasoningEffort: model.reasoningEffort, onReasoningChange: model.setReasoningEffort, searchMode, onSearchModeChange: setSearchMode, historyRetrieval, onHistoryRetrievalChange: setHistoryRetrieval, renderEnabled, onRenderEnabledChange: setRenderEnabled, disabled: !authorityReady, isLoading: generation.isActiveGenerating, onStop: generation.handleStop },
    },
  }
