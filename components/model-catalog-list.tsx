    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex min-w-0 items-center gap-2">
          ...
          {model.access === "premium" && <span className="shrink-0 rounded-md border border-border/80 bg-background/75 px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground dark:border-white/12 dark:bg-white/5">会员</span>}
...
function ProviderMark({ provider }: { provider: string }) {
  const label = provider === "OpenAI" ? "AI"
        ...
        : provider === "DeepSeek" ? "D"
          : provider === "Moonshot" ? "K"
            : provider === "MiniMax" ? "M"
              : "Z"
  return (
    <span className="flex size-9 shrink-0 items-center justify-center rounded-xl border border-border/70 bg-background text-[11px] font-semibold tracking-tight text-foreground shadow-[0_1px_3px_rgba(0,0,0,0.035)] dark:border-white/10 dark:bg-white/[0.045]">
      {label}
