...
function MdContent({ text }: { text: string }) {
  return (
    <ReactMarkdown
      ...
      components={{
        p: ({ children }) => <p className="mb-3 break-words leading-[1.9] tracking-[0.002em] [overflow-wrap:anywhere]">{children}</p>,
        ...
        h1: ({ children }) => <h1 className="mb-[13px] mt-8 text-[clamp(28px,1.7em,32px)] font-[700] leading-[1.16] tracking-[-0.016em]">{children}</h1>,
        h2: ({ children }) => <h2 className="mb-[11px] mt-7 text-[clamp(24px,1.42em,27px)] font-[650] leading-[1.22] tracking-[-0.014em]">{children}</h2>,
        h3: ({ children }) => <h3 className="mb-2.5 mt-6 text-[clamp(21px,1.22em,23px)] font-[600] leading-[1.28] tracking-[-0.012em]">{children}</h3>,
        h4: ({ children }) => <h4 className="mb-2 mt-5 text-[clamp(18px,1.08em,20px)] font-[600] leading-[1.34] tracking-[-0.01em]">{children}</h4>,
        h5: ({ children }) => <h5 className="mb-1.5 mt-4 text-[0.98em] font-[600] tracking-[-0.008em]">{children}</h5>,
        ...
        blockquote: ({ children }) => <blockquote className="my-3 rounded-r border-l-4 border-primary/45 bg-muted/20 py-2 pl-4 pr-3 font-[550] italic text-muted-foreground">{children}</blockquote>,
...
function ThinkingBlock({ thinking, active }: { thinking: string; active?: boolean }) {
  ...
  return (
    <div className="mb-4">
      <button
        onClick={() => setOpen(v => !v)}
        className="flex items-center gap-1.5 text-[13px] font-[550] italic text-muted-foreground/75 transition-colors hover:text-muted-foreground md:text-sm"
      >
        {open ? <ChevronDown className="size-3.5 md:size-4" /> : <ChevronRight className="size-3.5 md:size-4" />}
        <span className={active ? "thinking-flow not-italic font-[650] tracking-wide" : undefined}>thinking</span>
      </button>
      {open && (
        <div className="mt-2 break-words whitespace-pre-wrap rounded-xl border border-border/40 bg-muted/20 px-4 py-3 text-sm font-[500] italic leading-relaxed text-muted-foreground [overflow-wrap:anywhere] md:text-[15px]">
          {thinking}
        </div>
      )}
    </div>
        ...
        <div className="mt-2 space-y-2.5 rounded-xl border border-border/40 bg-muted/20 px-4 py-3 text-[13px] md:text-sm">
          {searches.map((s, i) => (
            <div key={i} className="space-y-1">
              <div className="text-xs font-[550] italic text-muted-foreground md:text-[13px]">搜索：{s.query}</div>
              {s.results.map((r, j) => (
                <a key={j} href={r.url} target="_blank" rel="noreferrer" className="block truncate text-xs font-[550] text-primary/80 underline underline-offset-2 hover:text-primary md:text-[13px]">
                  {r.title || r.url}
                </a>
              ))}
            </div>
            ...
            <div key={m.id} className="flex flex-col items-end">
              ...
              {m.content && (
                <div className="max-w-[90%] min-w-0 rounded-[1.5rem] rounded-tr-md user-bubble-bg px-5 py-3.5 md:max-w-[85%]">
                  <p className="break-words text-[17px] font-[550] italic leading-[1.86] tracking-[0.002em] text-secondary-foreground [overflow-wrap:anywhere] md:text-[18px]">{m.content}</p>
                </div>
              )}
            </div>
          ) : (
                ...
                {m.memoryNotes && m.memoryNotes.length > 0 && (
                  <div className="mb-3 space-y-1">
                    {m.memoryNotes.map((note, i) => (
                      <div key={i} className="flex items-center gap-1.5 text-[13px] font-[550] italic text-muted-foreground/80 md:text-sm">
                ...
                {(() => {
                  ...
                  return (
                    <div className="min-w-0 space-y-3">
                      {m.isError ? (
                        <div>
                          <p className="break-words whitespace-pre-wrap text-sm font-[500] italic leading-relaxed text-muted-foreground [overflow-wrap:anywhere] md:text-[15px]">{m.content}</p>
                        </div>
                      ) : (
                        <>
                          {/* 字号回到可读区间；取消 font-size-adjust，避免英文和整体视觉被压小 */}
                          {display && (
                            <div className="text-[17px] font-[550] text-foreground md:text-[18px]">
