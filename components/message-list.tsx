...
function MdContent({ text }: { text: string }) {
  return (
    <ReactMarkdown
      ...
      components={{
        p: ({ children }) => <p className="mb-3 break-words leading-[1.88] tracking-[0.001em] [overflow-wrap:anywhere]">{children}</p>,
        ...
        h1: ({ children }) => <h1 className="mb-3 mt-8 text-[clamp(24px,1.6em,28px)] font-[700] leading-[1.18] tracking-[-0.014em]">{children}</h1>,
        h2: ({ children }) => <h2 className="mb-2.5 mt-7 text-[clamp(21px,1.35em,24px)] font-[650] leading-[1.24] tracking-[-0.012em]">{children}</h2>,
        h3: ({ children }) => <h3 className="mb-2 mt-6 text-[clamp(18px,1.2em,21px)] font-[600] leading-[1.3] tracking-[-0.01em]">{children}</h3>,
        h4: ({ children }) => <h4 className="mb-2 mt-5 text-[clamp(16px,1.08em,18px)] font-[600] leading-[1.34] tracking-[-0.008em]">{children}</h4>,
        h5: ({ children }) => <h5 className="mb-1.5 mt-4 text-[0.98em] font-[600] tracking-[-0.006em]">{children}</h5>,
        ...
        blockquote: ({ children }) => <blockquote className="my-3 rounded-r border-l-4 border-primary/45 bg-muted/20 py-2 pl-4 pr-3 font-[400] italic text-muted-foreground">{children}</blockquote>,
        ...
        thead: ({ children }) => <thead className="bg-muted/40 font-[600]">{children}</thead>,
        tbody: ({ children }) => <tbody>{children}</tbody>,
        tr: ({ children }) => <tr className="border-b border-border/20 last:border-b-0">{children}</tr>,
        th: ({ children }) => <th className="border-r border-border/20 px-3 py-2 text-left text-sm font-[600] last:border-r-0">{children}</th>,
...
function ThinkingBlock({ thinking, active }: { thinking: string; active?: boolean }) {
  ...
  return (
    <div className="mb-4">
      <button
        onClick={() => setOpen(v => !v)}
        className="flex items-center gap-1.5 text-xs font-[500] italic text-muted-foreground/75 transition-colors hover:text-muted-foreground md:text-[13px]"
      >
        {open ? <ChevronDown className="size-3 md:size-3.5" /> : <ChevronRight className="size-3 md:size-3.5" />}
        <span className={active ? "thinking-flow not-italic font-[600] tracking-wide" : undefined}>thinking</span>
      </button>
      {open && (
        <div className="mt-2 break-words whitespace-pre-wrap rounded-xl border border-border/40 bg-muted/20 px-4 py-3 text-[13px] font-[400] italic leading-relaxed text-muted-foreground [overflow-wrap:anywhere] md:text-sm">
          {thinking}
        </div>
      )}
    </div>
...
function SearchBlock({ searches, replying }: { searches: { query: string; results: { title: string; url: string }[] }[]; replying: boolean }) {
  ...
  return (
    <div className="mb-3">
      ...
      >
        {open ? <ChevronDown className="size-3 md:size-3.5" /> : <ChevronRight className="size-3 md:size-3.5" />}
        <Globe className="size-3 md:size-3.5" />
        <span>搜索了 {total} 个来源</span>
      </button>
      {open && (
        <div className="mt-2 space-y-2.5 rounded-xl border border-border/40 bg-muted/20 px-4 py-3 text-xs md:text-[13px]">
          {searches.map((s, i) => (
            ...
            <div key={m.id} className="flex flex-col items-end">
              ...
              {m.content && (
                <div className="max-w-[90%] min-w-0 rounded-[1.5rem] rounded-tr-md user-bubble-bg px-5 py-3.5 md:max-w-[85%]">
                  <p className="break-words text-[15px] font-[400] italic leading-[1.88] tracking-[0.001em] text-secondary-foreground [overflow-wrap:anywhere] md:text-[16px]">{m.content}</p>
                </div>
              )}
            </div>
          ) : (
            <div key={m.id} className="group grid min-w-0 grid-cols-[1.625rem_minmax(0,1fr)] items-start gap-2 md:grid-cols-[2.75rem_minmax(0,1fr)] md:gap-2.5">
              <div className="avatar-box mt-0.5 h-[26px] w-[26px] flex-shrink-0 self-start md:h-10 md:w-10">
                <Image src="/companion.png" alt="" width={40} height={40} priority className="avatar-light size-[26px] select-none md:size-10" />
                <Image src="/companion-dark.png" alt="" width={40} height={40} priority className="avatar-dark size-[26px] select-none md:size-10" />
              </div>
                ...
                {m.memoryNotes && m.memoryNotes.length > 0 && (
                  <div className="mb-3 space-y-1">
                    {m.memoryNotes.map((note, i) => (
                      <div key={i} className="flex items-center gap-1.5 text-xs font-[500] italic text-muted-foreground/80 md:text-[13px]">
                        <Brain className="size-3 shrink-0 md:size-3.5" />
                ...
                {(() => {
                  ...
                  return (
                    <div className="min-w-0 space-y-3">
                      {m.isError ? (
                        <div>
                          <p className="break-words whitespace-pre-wrap text-[13px] font-[400] italic leading-relaxed text-muted-foreground [overflow-wrap:anywhere] md:text-sm">{m.content}</p>
                        </div>
                      ) : (
                        <>
                          {/* 正文回到 400，整体字号收小；font-size-adjust 让英文更贴近中文比例 */}
                          {display && (
                            <div className="text-[15px] font-[400] text-foreground [font-size-adjust:0.47] md:text-[16px]">
